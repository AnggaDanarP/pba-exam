...
pub fn answer_1_a() -> char {
	'd' // Rust defaults to i32 for integer literals unless specified otherwise.
}
...
pub fn answer_1_b() -> char {
	'c' // Rust allows variable shadowing where a new variable with the same name can be declared, which shadows the previous variable.
}
...
pub fn answer_1_d() -> char {
	'a' // Rust does not support the do-while loop; it supports loop, while, and for loops.
}
...
pub fn answer_2() -> char {
	'e' // The variable result is declared within each block of the if and else if statements, making it out of scope at the return statement.
}
...
pub fn answer_3_a() -> char {
	'a' // The function computes squares of numbers from 1 to 5 and adds 3 to each result.
}
...
pub fn answer_3_b() -> char {
	'd' // This is essentially the same as the original range 1..n, iterating the same number of times.
}
...
pub fn answer_4_a() -> char {
	'd' // The function signature indicates that it modifies the input slice in place.
}
...
pub fn answer_4_b() -> char {
	'b' // The Ord trait is required to compare elements for sorting.
}
...
pub fn answer_7() -> char {
	'e' // The ? operator can convert ParseIntError to OutOfRangeError using the From trait implementation.
}
